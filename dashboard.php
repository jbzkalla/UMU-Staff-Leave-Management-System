...
if(!isset($_SESSION['staff-user']) && !isset($_SESSION['supervisor-user'])){
    echo "<script>location.href = 'index.php';</script>";
    exit();
}else{


    $staff_username = isset($_SESSION['staff-user']) ? $_SESSION['staff-user'] : $_SESSION['supervisor-user'];
    $staff_email = isset($_SESSION['staff-email']) ? $_SESSION['staff-email'] : $_SESSION['supervisor-email'];
    $staff_id = isset($_SESSION['staff-id']) ? $_SESSION['staff-id'] : $_SESSION['supervisor-id'];


    $stmt = $db_con->prepare("SELECT * FROM employee WHERE username = ?");
...
PHP;
        ...
        $stmt = $db_con->prepare("SELECT * FROM rejected_leaves WHERE staff_id = ?");
...
PP;
        ...
        $stmt = $db_con->prepare("SELECT * FROM accepted_leaves WHERE staff_id = ?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $result = $stmt->get_result();
...
GROUP_ONE;
            ...
            echo '<h2 class="text-center mb-5">No pending data available</h2>';
        }
        ...
        $stmt = $db_con->prepare("SELECT * FROM job_description WHERE staff_id = ?");
        $stmt->bind_param("i", $staff_id);
        $stmt->execute();
        $stmt_res = $stmt->get_result();

        $rows_count = $stmt_res->num_rows;
        ...
        if($rows_count > 0){
            ...
            while($row = $stmt_res->fetch_object()){
...
if($result->num_rows > 0){
    ...
    while ($row = $result->fetch_object()){
        ...
        $status = "<button class='btn success-btn'>"
            ...
            $stage = $row->approval_stage ? $row->approval_stage : "Pending Supervisor";
            $status = "<button class='btn pending-btn'>"
                    . "<i class='fa fa-hourglass-half pr-2'></i> $stage</button>";
        }
...
echo '</table></div>';
        ...
        $welcome_name = (is_object($res) ? "$res->fname $res->lname" : $staff_username);
        $kamil = <<<FMS
            <div class="container mb-5 p-4">
                <h4 class="text-center">
                Welcome, $welcome_name</h4>
                ...
                <h5>Uganda Martyrs University – Staff Portal</h5>
