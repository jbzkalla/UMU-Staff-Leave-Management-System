...
$stmt = $db_con->prepare("SELECT la.* FROM leave_applications la 
        JOIN employee e ON la.staff_id = e.staff_id 
        WHERE la.action IS NULL AND e.supervisor = ?");
$stmt->bind_param("s", $username);
$stmt->execute();
$result = $stmt->get_result();

echo '<div class="card mb-md-5">
    <h1 class="text-md text-center">Staff Requests Pending Recommendation</h1>
...
if($result->num_rows > 0){
    ...
    while ($row = $result->fetch_object()){
        ...
        $start_date = new DateTime($row->leave_start_date);
        $end_date = new DateTime($row->leave_end_date);
        $interval = $start_date->diff($end_date);
        $num_days = $interval->days + 1; // Include both start and end dates
        
        $stmt = $db_con->prepare("SELECT fname,email FROM employee WHERE staff_id = ?");
        $stmt->bind_param("i", $row->staff_id);
        $stmt->execute();
        $rows = $stmt->get_result()->fetch_object();
