 ...
 * @global mysqli $db_con The connection handle
 ...
 * @return bool True if item in table. False if item is not in table
 */
function in_db($table, $column, $item) {
 ...
 * @global mysqli $handle The connection handle to the database
 * @param mysqli $handle The connection handle
 ...
 * @global mysqli $db_con - The database connection handle.
 * @param string $str - The query statement/string
 * @return object|false - Returns object on query success or false otherwise
 */
function query_db(string $str) {  
        
    global $db_con;
        
    $res = $db_con->query($str);
        
    if(!$res || !is_object($res) || $res->num_rows < 1 || $db_con->affected_rows == 0){
    ...
    return is_object($rows) ? $rows : FALSE;
 }
    
/**
 * 
 * @param string $tbl is table name to select from
 ...
 * @return bool|object - Returns @param $rows populated with data for use upon success
 * or FALSE on failure
 */
 ...
 * @global mysqli $db_con Connection handle
 */
 ...
 * @param mixed $var - The variable we're testing
 ...
 * based  on its value
 * @param object $rows: the object containing user details
 */
function determine_user_set_session($user_type, $rows) {
    
    if(!is_object($rows)) return;
    ...
    }elseif ($user_type == "employee" || $user_type == "staff") {
...
function verify_redirrect_user($user, $password, $tbl, $page) {
    ...
    if(is_object($rows)){
        ...
        if(password_verify($rows->password, PASSWORD_DEFAULT)){
            ...
            determine_user_set_session($tbl, $rows);
 ...
 * @return bool - True if field is empty, false otherwise
 */
 ...
 * @return bool Returns true if message is sent. False otherwise
 */
