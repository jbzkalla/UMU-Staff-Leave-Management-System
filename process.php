...
if(isset($_POST['register'])){
    ...
    if(!isset($_POST['payroll']) || $_POST['payroll'] == ''){
        $errors[] = urlencode('Payroll Number is required');
    } else{
        $payroll = strip_tags(trim(htmlspecialchars($_POST['payroll'])));
    }
    
    if(!isset($_POST['department']) || $_POST['department'] == ''){
        $errors[] = urlencode('Department is required');
    } else{
        $dept_id = intval($_POST['department']);
    }
    
    if(!isset($_POST['rank']) || $_POST['rank'] == ''){
        $errors[] = urlencode('Position/Rank is required');
    } else{
        $rank = strip_tags(trim(htmlspecialchars($_POST['rank'])));
    }
    ...
    $stmt = $db_con->prepare("SELECT id FROM employee WHERE username = ? OR email = ? OR phone = ?");
    ...
    if(!$errors){
        ...
        $stmt = $db_con->prepare("INSERT INTO employee(staff_id,payroll_number,department_id,position_or_rank,title,fname,
            lname,username,password,email,country_code,phone,date_registered) 
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)");
        
        $stmt->bind_param('ssissssssssis',$staff_id, $payroll, $dept_id, $rank, $title, $firstname, $lastname,
        ...
        if($row == 1){
            ...
            $msg = urlencode("Congratulations $lastname! Your account for Leon SmartLeave at Uganda Martyrs University has been successfully created. Please wait for HR Admin approval before logging in.");
                
            header('Location:thankyou.php?msg='.$msg);
            
            $subject = "Registration Successful - UMU Staff Leave Manager";
            $raw_message = "Hi $username!
                \t\tThis is to inform you that your account has been created successfully and is waiting for HR Admin approval. Once you have been approved, you can apply for leave and manage your records.\n\nBest Regards,\nHR Admin\nUganda Martyrs University";
            ...
            header('Location:register.php?error='.implode("<br>", $errors));
    ...
    // SECURITY: Ensure ONLY HR Admin can perform this
    if(!isset($_SESSION['admin-user'])){
        redirect_user("index.php?error=".urlencode("Unauthorized action."));
    }
    ...
    if(var_set($_POST['make-supervisor'])){
        ...
        if(is_string($_POST['make-supervisor'])){
            ...
            $stmt = $db_con->prepare("UPDATE employee SET supervisor = 'N/A', staff_level = 'supervisor' WHERE username = ?");
            $stmt->bind_param("s", $make_super);
            $stmt->execute();
    ...
    if(!$error){
        ...
        $stmt = $db_con->prepare("UPDATE employee SET supervisor = ?, staff_level = 'non-supervisor' WHERE username = ?");
        $stmt->bind_param("ss", $super, $username);
        $stmt->execute();
        ...
        redirect_user("admin.php?tab=5&error=".implode("<br>", $error));
    }
    
}elseif(isset($_POST['accept'])){
    
    // SECURITY: Ensure user is authorized (Supervisor or Admin)
    if(!isset($_SESSION['supervisor-user']) && !isset($_SESSION['admin-user'])){
         redirect_user("index.php?error=".urlencode("Unauthorized action."));
    }
    ...
    if(!$error){
        ...
        $result = $db_con->query("UPDATE leave_applications SET action = 'accept', approval_stage = 'Final Approval' 
    ...
    if(!$err){
        ...
        redirect_user("admin.php?error=".implode("<br>", $err));
    } 
    ...
    if(!$errors){
        ...
        redirect_user("admin.php?tab=6&error=".implode("<br>", $errors));
    }
    ...
    if(!$errors){
        ...
        redirect_user("admin.php?tab=8&error=".implode("<br>", $errors));        
    }
    ...
    if(!$error){
        ...
        $result = $db_con->query("UPDATE leave_applications SET action = 'reject', approval_stage = 'Rejected' 
            WHERE leave_id = $leave_id AND staff_id = $staff_id");
        ...
        redirect_user("admin.php?tab=3&error=".implode("<br>", $error));
