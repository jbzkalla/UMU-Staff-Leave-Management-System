...
if(isset($_POST['accept'])){
    ...
    if(!$error){
        ...
        $stmt = $db_con->prepare("UPDATE leave_applications SET action = 'accept', approval_stage = 'Pending HR' WHERE leave_id = ? AND staff_id = ?");
        $stmt->bind_param("ii", $leave_id, $staff_id);
        $stmt->execute();
        $result = $stmt; // Using $stmt for affected_rows check below
        ...
        if($db_con->affected_rows == 1){
           ...
           if($db_con->affected_rows == 1){
               ...
               $from_email = "hrm@umu.ac.ug";
               $headers = "From: UMU HR Office <$from_email>\r\n";
        ...
        redirect_user("dashboard.php?tab=7&error=".implode("<br>", $error));
    }
    ...
    if(!$error){
        ...
        $stmt = $db_con->prepare("UPDATE leave_applications SET action = 'reject', approval_stage = 'Rejected' WHERE leave_id = ? AND staff_id = ?");
        $stmt->bind_param("ii", $leave_id, $staff_id);
        $stmt->execute();
        $result = $stmt;
